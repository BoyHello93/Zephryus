...
export const getSettingsFromStorage = (): AppSettings => {
  try {
    ...
    if (!data) return { showTips: true, showInsertionHints: true, theme: 'zephyr' };
    const parsed = JSON.parse(data);
    return {
        showTips: parsed.showTips ?? true,
        showInsertionHints: parsed.showInsertionHints ?? true,
        theme: parsed.theme ?? 'zephyr'
    };
  } catch (e) {
    return { showTips: true, showInsertionHints: true, theme: 'zephyr' };
  }
};
